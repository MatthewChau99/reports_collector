...
class SouHu:
    def __init__(self, search_word, s_date, max_art, max_text=3000):
        ...
        self.path = os.path.join(ROOT_DIR, 'cache', search_word, 'news', 'souhu')
        self.driver = webdriver.Chrome(os.path.join(ROOT_DIR, 'venv', 'chromedriver'))
    ...
    def get_content(self, url):
        ...
        if len(text) > self.max_text:
            ...
            json_result['doc_id'] = url.replace('http://www.sohu.com/a/', '')
            try:
