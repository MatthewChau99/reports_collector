...
class SouGou:
    ...
    def __init__(self, search_word, s_date, max_art, max_text=3000):
        ...
        self.path = os.path.join(ROOT_DIR, 'cache', search_word, 'news', 'sougou')
        self.result = []
        self.main()
        self.driver.close()
    ...
    def get_content(self, url):
        ...
        if len(text) > self.max_text:
            ...
            json_result = {'source': 'sougou', 'doc_id': '', 'date': '', 'download_url': url, 'org_name': '',
                           'page_num': '1', 'doc_type': 'NEWS', 'title': ''}
            json_result['doc_id'] = url.replace('https://weixin.sogou.com/link?', '')
            try:
                json_result['date'] = ''.join(
                    self.driver.find_element_by_xpath('//em[@id="publish_time"]').text.split())
                if '-' not in json_result['date']:
                    json_result['date'] = time.strftime("%Y-%m-%d", time.localtime())
